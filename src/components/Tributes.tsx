      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 md:gap-6">
          {CHILD_TRIBUTES.map((tribute, index) => (
            <motion.article key={tribute.name} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: index * 0.06 }} className="gold-card rounded-2xl overflow-hidden">
              <div className="h-1" style={{ background: 'linear-gradient(to right, #8b6914, #d4af37, #ffe082, #d4af37, #8b6914)' }} />
              <div className="p-6 md:p-8">
                <div className="flex items-center gap-3 mb-5">
                  <div className="w-11 h-11 rounded-full flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #2d1f00, #d4af37)' }}><Heart className="w-5 h-5 text-amber-200 fill-amber-200/20" /></div>
                  <h3 className="font-heading font-bold text-xl md:text-2xl text-[#2d1f00]">{tribute.name}</h3>
                  {index === 0 && <Star className="w-4 h-4 text-amber-500 fill-amber-500 ml-auto" />}
                </div>
